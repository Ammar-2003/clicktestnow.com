document.addEventListener('DOMContentLoaded', function() {
    ...
    const SESSION_SHOWN_KEY = 'loginPromptShownThisSession_15s';
    const LAST_TEST_SCORE_KEY = 'last15sCpsTestScore';
    const LAST_TEST_CLICKS_KEY = 'last15sCpsTestClicks';
    ...
    // Global state
    let testDuration = 15; // Changed to 15 seconds
    ...
    function loadLeaderboard() {
        fetch("/get-15second-cps-leaderboard/")
            .then(res => res.json())
            .then(data => {
    ...
    async function endTest() {
        ...
        // Calculate additional metrics for 15-second test
        let peakCpsValue = 0;
        ...
        if (clickTimes.length > 1) {
            ...
            // Split into 3 segments of 5 seconds each for 15-second test
        ...
        // Determine click technique based on pattern for 15-second test
        ...
        // Generate improvement tip for 15-second test
        let tip = "";
        if (cps < 6) {
            tip = "Focus on developing a consistent rhythm. Try different clicking techniques to find what works best for you over 15 seconds.";
        } else if (cps < 8) {
            tip = "Your clicking speed is good. Work on maintaining consistency throughout the entire 15 seconds. Consider alternating fingers for better endurance.";
        } else if (cps < 10) {
            tip = "Excellent speed! For 15-second tests, focus on pacing yourself. Try to maintain 80% of your peak speed throughout.";
        } else if (cps < 12) {
            tip = "You're an elite clicker! Focus on endurance training to maintain your high speed for the full 15 seconds.";
        } else {
            tip = "You're at a professional level! Your 15-second endurance is exceptional. Share your techniques and consider competing in click speed tournaments.";
        }
    ...
    function resetTest() {
        ...
        if (statusDisplay) {
            statusDisplay.innerHTML = `
                ...
                <div>Click to start 15-second test</div>
            `;
        }
    ...
    // Save score to database for 15-second test
    function saveScoreToDatabase(score, clicks, peakCps, endurance) {
        fetch('/save-15-second-cps-score/', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.status === 'success') {
                console.log('15-second score saved successfully');
                if (userPosition) {
        ...
        .catch(error => {
            console.error('Error saving 15-second score:', error);
            showNotification('Failed to save score. Please try again.', 'error');
        });
    }
    
    // Show achievement animation for 15-second test
    function showAchievementAnimation(rank, score, clicks) {
        ...
        if (rank === 1) {
            ...
            message = `You've achieved the #1 spot globally with ${score.toFixed(1)} CPS over 15 seconds! Your speed is unmatched.`;
        } else if (rank === 2) {
            icon = '🥈';
            title = 'SILVER MEDALIST!';
            message = `Amazing 15-second performance! You're the 2nd best clicker worldwide with ${score.toFixed(1)} CPS.`;
        } else if (rank === 3) {
            icon = '🥉';
            ...
            icon = '⭐';
            title = 'TOP 4 ELITE!';
            message = `Incredible 15-second performance! You're among the top 4 clickers worldwide with ${score.toFixed(1)} CPS.`;
        } else if (rank <= 10) {
            icon = '🏆';
            title = 'TOP 10 MASTER!';
        ...
        overlay.innerHTML = `
            <div class="achievement-content">
                ...
                <div class="achievement-rank">Rank: #${rank} | Score: ${score.toFixed(1)} CPS | Clicks: ${clicks} | Duration: 15s</div>
