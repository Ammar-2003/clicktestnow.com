document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("/get-spacebar-leaderboard20/")
        .then(res => res.json())
        .then(data => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Test variables - 20 seconds for spacebar test
    let testDuration = 20;
    ...
    // New flag to prevent continuous presses from a single keydown event
    ...
    document.addEventListener('keydown', function(e) {
        if (e.code === 'Space' || e.keyCode === 32) {
            ...
            // Only register press if the key is not already held down
            if (testState === 'active' && !isSpacebarHeld) {
                registerPress();
                isSpacebarHeld = true;
            }
        }
    ...
    function startTest() {
        ...
        testArea.classList.add('keyboard-focus');
        testArea.focus();
    ...
    function endTest() {
        ...
        testArea.classList.remove('keyboard-focus');
        ...
        if (pressTimes.length > 1) {
            ...
            // Calculate peak PPS (highest 1-second rate)
            peakPpsValue = maxPpsValue;
        }
        
        // For 20-second spacebar test, stamina is calculated based on consistency
        const staminaScoreValue = Math.min(100, consistencyValue * 0.8 + (pressCount > 200 ? 20 : 0));
        ...
        if (pressTimes.length > 5) {
            ...
            if (ppsRate > 14) {
                ...
                techniqueTags = '<span class="technique-tag">High Speed</span><span class="technique-tag">Rhythmic</span>';
            } else if (ppsRate > 10) {
                technique = "Fast Pressing";
                techniqueTags = '<span class="technique-tag">Medium Speed</span><span class="technique-tag">Consistent</span>';
            } else if (ppsRate > 7) {
                technique = "Moderate Pressing";
                techniqueTags = '<span class="technique-tag">Steady Pace</span><span class="technique-tag">Endurance</span>';
            } else {
                technique = "Standard Pressing";
                techniqueTags = '<span class="technique-tag">Basic</span><span class="technique-tag">Comfortable Pace</span>';
            }
        }
        ...
        let tip = "";
        if (pps < 8) {
            tip = "Focus on developing a consistent rhythm rather than pressing randomly. Try using your thumb for better control.";
        } else if (pps < 12) {
            tip = "Good speed! Try to relax your hand more while maintaining the rhythm. Tension can reduce your pressing speed.";
        } else if (pps < 16) {
            tip = "Excellent speed! Work on building stamina to maintain your technique for the full 20 seconds.";
        } else if (pps < 20) {
            tip = "Your spacebar technique is impressive. Experiment with different hand positions to find what works best.";
        } else if (pps < 25) {
    ...
    function showAchievementAnimation(rank, score, presses) {
        ...
        overlay.innerHTML = `
            <div class="achievement-content">
                ...
                <div class="achievement-rank">Rank: #${rank} | Score: ${score} Presses | PPS: ${(score/20).toFixed(1)}</div>
