document.addEventListener('DOMContentLoaded', function() {
    ...
    const SESSION_SHOWN_KEY = 'loginPromptShownThisSession_2s';
    const LAST_TEST_SCORE_KEY = 'last2sCpsTestScore';
    const LAST_TEST_CLICKS_KEY = 'last2sCpsTestClicks';
    ...
    function loadLeaderboard() {
        fetch("/get-2-second-leaderboard/")
            .then(res => res.json())
            .then(data => {
    ...
    async function endTest() {
        ...
        // Calculate additional metrics for 2-second test
        let peakCpsValue = 0;
        let enduranceValue = 100;
        
        if (clickTimes.length > 1) {
            // For 2-second test, calculate peak CPS in any 0.5-second window (scaled to 1 second)
            const halfSecond = 500;
            let startIdx = 0;
            
            for (let i = 0; i < clickTimes.length; i++) {
                while (clickTimes[i] - clickTimes[startIdx] > halfSecond) {
                    startIdx++;
                }
                peakCpsValue = Math.max(peakCpsValue, (i - startIdx + 1) * 2); // Scale to 1 second
            }
            
            // For 2-second test, calculate consistency across the entire duration
            const intervals = [];
        ...
        // Determine click technique based on pattern for 2-second test
        ...
        // Generate improvement tip for 2-second test
        let tip = "";
        if (frozenCps < 7) {
            tip = "Focus on explosive clicking power. For 2-second tests, you need maximum speed from the very first click.";
        } else if (frozenCps < 10) {
            tip = "Your clicking speed is good. Work on starting strong from the very first click. Every millisecond counts in 2-second tests.";
        } else if (frozenCps < 13) {
            tip = "Excellent speed! For 2-second tests, focus on maintaining your peak speed without any hesitation.";
        } else if (frozenCps < 16) {
            tip = "You're an elite clicker! Focus on explosive clicking techniques for maximum speed in 2-second bursts.";
        } else {
            tip = "You're at a professional level! Your 2-second explosive speed is exceptional. This is perfect for competitive gaming scenarios.";
        }
    ...
    function resetTest() {
        ...
        if (statusDisplay) {
            statusDisplay.innerHTML = `
                ...
                <div>Click to start 2-second test</div>
            `;
        }
    ...
    // Save score to database for 2-second test
    function saveScoreToDatabase(score, clicks, peakCps, endurance) {
        fetch('/save-2-second-score/', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.status === 'success') {
                console.log('2-second score saved successfully');
                if (userPosition) {
        ...
        .catch(error => {
            console.error('Error saving 2-second score:', error);
            showNotification('Failed to save score. Please try again.', 'error');
        });
    }
    
    // Show achievement animation for 2-second test
    function showAchievementAnimation(rank, score, clicks) {
        ...
        if (rank === 1) {
            ...
            message = `You've achieved the #1 spot globally with ${score.toFixed(1)} CPS over 2 seconds! Your explosive speed is unmatched.`;
        } else if (rank === 2) {
            icon = '🥈';
            title = 'SILVER MEDALIST!';
            message = `Amazing 2-second performance! You're the 2nd best clicker worldwide with ${score.toFixed(1)} CPS.`;
        } else if (rank === 3) {
            icon = '🥉';
            ...
            icon = '⭐';
            title = 'TOP 4 ELITE!';
            message = `Incredible 2-second performance! You're among the top 4 clickers worldwide with ${score.toFixed(1)} CPS.`;
        } else if (rank <= 10) {
            icon = '🏆';
            title = 'TOP 10 MASTER!';
        ...
        overlay.innerHTML = `
            <div class="achievement-content">
                ...
                <div class="achievement-rank">Rank: #${rank} | Score: ${score.toFixed(1)} CPS | Clicks: ${clicks} | Duration: 2s</div>
