document.addEventListener('DOMContentLoaded', function() {
    ...
    const SESSION_SHOWN_KEY = 'loginPromptShownThisSession_10s';
    const LAST_TEST_SCORE_KEY = 'last10sCpsTestScore';
    const LAST_TEST_CLICKS_KEY = 'last10sCpsTestClicks';
    ...
    // Global state
    let testDuration = 10; // 10 seconds
    ...
    function loadLeaderboard() {
        fetch("/get-10-second-leaderboard/")
            .then(res => res.json())
            .then(data => {
    ...
    async function endTest() {
        ...
        // Calculate additional metrics for 10-second test
        let peakCpsValue = 0;
        ...
        if (clickTimes.length > 1) {
            ...
            // Split into 2 segments of 5 seconds each for 10-second test
        ...
        // Determine click technique based on pattern for 10-second test
        ...
        // Generate improvement tip for 10-second test
        let tip = "";
        if (cps < 6) {
            tip = "Focus on developing a consistent rhythm. Try different clicking techniques to find what works best for you over 10 seconds.";
        } else if (cps < 8) {
            tip = "Your clicking speed is good. Work on maintaining consistency throughout the entire 10 seconds. Consider alternating fingers for better endurance.";
        } else if (cps < 10) {
            tip = "Excellent speed! For 10-second tests, focus on pacing yourself. Try to maintain 80% of your peak speed throughout.";
        } else if (cps < 12) {
            tip = "You're an elite clicker! Focus on endurance training to maintain your high speed for the full 10 seconds.";
        } else {
            tip = "You're at a professional level! Your 10-second endurance is exceptional. Share your techniques and consider competing in click speed tournaments.";
        }
    ...
    function resetTest() {
        ...
        if (statusDisplay) {
            statusDisplay.innerHTML = `
                ...
                <div>Click to start 10-second test</div>
            `;
        }
    ...
    // Save score to database for 10-second test
    function saveScoreToDatabase(score, clicks, peakCps, endurance) {
        fetch('/save-10-second-score/', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.status === 'success') {
                console.log('10-second score saved successfully');
                if (userPosition) {
        ...
        .catch(error => {
            console.error('Error saving 10-second score:', error);
            showNotification('Failed to save score. Please try again.', 'error');
        });
    }
    
    // Show achievement animation for 10-second test
    function showAchievementAnimation(rank, score, clicks) {
        ...
        if (rank === 1) {
            ...
            message = `You've achieved the #1 spot globally with ${score.toFixed(1)} CPS over 10 seconds! Your speed is unmatched.`;
        } else if (rank === 2) {
            icon = '🥈';
            title = 'SILVER MEDALIST!';
            message = `Amazing 10-second performance! You're the 2nd best clicker worldwide with ${score.toFixed(1)} CPS.`;
        } else if (rank === 3) {
            icon = '🥉';
            ...
            icon = '⭐';
            title = 'TOP 4 ELITE!';
            message = `Incredible 10-second performance! You're among the top 4 clickers worldwide with ${score.toFixed(1)} CPS.`;
        } else if (rank <= 10) {
            icon = '🏆';
            title = 'TOP 10 MASTER!';
        ...
        overlay.innerHTML = `
            <div class="achievement-content">
                ...
                <div class="achievement-rank">Rank: #${rank} | Score: ${score.toFixed(1)} CPS | Clicks: ${clicks} | Duration: 10s</div>
