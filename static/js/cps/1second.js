document.addEventListener('DOMContentLoaded', function() {
    ...
    if (leaderboardContent) {
        fetch("/get-one-second-cps-leaderboard/")
            .then(res => res.json())
            .then(data => {
    ...
    // Test variables - set to 1 second for 1-second CPS test
    let testDuration = 1;
        ...
        countdownInterval = setInterval(() => {
            ...
            if (countdown > 0 && statusDisplay) {
                ...
                if (testState === 'countdown') {
                    ...
                    // Updated to 1-second text for 1-second CPS test
                    if (statusDisplay) {
                        statusDisplay.innerHTML = `
    ...
    function endTest() {
        ...
        // For 1-second CPS test, stamina is calculated differently
        const staminaScoreValue = Math.min(100, consistencyValue * 0.8 + (clickCount > 10 ? 20 : 0));
        ...
        let tip = "";
        if (cps < 7) {
            tip = "Focus on reducing the time between clicks. Try relaxing your hand for faster movement.";
        } else if (cps < 10) {
            tip = "Good speed! Try using two fingers (butterfly clicking) to increase your CPS further.";
        } else if (cps < 14) {
            tip = "Excellent speed! Work on consistency to maintain this rate for longer periods.";
        } else if (cps < 18) {
            tip = "You're an expert clicker! Consider learning jitter clicking techniques to reach even higher CPS.";
        } else if (cps < 21) {
            tip = "Amazing speed! You're among the top clickers. Focus on maintaining consistency.";
        } else {
            tip = "You're at an elite clicking level! Your technique is exceptional.";
        }
    ...
    function resetTest() {
        ...
        if (statusDisplay) {
            statusDisplay.innerHTML = `
                ...
                <div>Click to start 1-second test</div>
            `;
        }
