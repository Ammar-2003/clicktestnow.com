document.addEventListener('DOMContentLoaded', function() {
    ...
    const SESSION_SHOWN_KEY = 'loginPromptShownThisSession_1s';
    const LAST_TEST_SCORE_KEY = 'last1sCpsTestScore';
    const LAST_TEST_CLICKS_KEY = 'last1sCpsTestClicks';
    ...
    async function endTest() {
        ...
        if (clickTimes.length > 1) {
            // For 1-second test, calculate peak CPS in any 0.2-second window (scaled to 1 second)
            const fifthSecond = 200;
            let startIdx = 0;
            
            for (let i = 0; i < clickTimes.length; i++) {
                while (clickTimes[i] - clickTimes[startIdx] > fifthSecond) {
                    startIdx++;
                }
                peakCpsValue = Math.max(peakCpsValue, (i - startIdx + 1) * 5); // Scale to 1 second
            }
            
            // For 1-second test, calculate consistency across the entire duration
            const intervals = [];
        ...
        // Determine click technique based on pattern for 1-second test
