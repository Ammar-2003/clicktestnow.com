document.addEventListener('DOMContentLoaded', function() {
    ...
    const SESSION_SHOWN_KEY = 'loginPromptShownThisSession_spacebar';
    const LAST_TEST_SCORE_KEY = 'lastSpacebarTestScore';
    const LAST_TEST_PRESSES_KEY = 'lastSpacebarTestPresses';
    ...
    // Global state
    let testDuration = 3; // 3 seconds for spacebar test
    ...
    function loadLeaderboard() {
        fetch("/get-spacebar-leaderboard/")
            .then(res => res.json())
            .then(data => {
    ...
    document.addEventListener('keydown', function(e) {
        if (e.code === 'Space' && !e.repeat) {
            ...
            if (testState === 'idle' || testState === 'ended') {
                ...
                startTest();
            } else if (testState === 'active' && !isKeyDown) {
                isKeyDown = true;
                registerPress();
            }
        }
    ...
    async function endTest() {
        ...
        // Calculate additional metrics for spacebar test
        let maxPpsValue = 0;
        ...
        if (pressTimes.length > 1) {
            ...
            // For 3-second spacebar test, stamina is calculated differently
            staminaScoreValue = Math.min(100, consistencyValue * 0.8 + (frozenPresses > (testDuration * 15) ? 20 : 0));
        }
        ...
        if (frozenPps < 9) {
            ...
            tip = "Work on building stamina to maintain your pressing technique for the full 3 seconds.";
    ...
    function saveScoreToDatabase(score, presses, maxPps, consistency, stamina, reactionTime) {
        ...
        .then(data => {
            if (data.status === 'success') {
                console.log('Spacebar score saved successfully');
                if (userPosition) {
        ...
        .catch(error => {
            console.error('Error saving spacebar score:', error);
            showNotification('Failed to save score. Please try again.', 'error');
        });
    }
    
    // Show achievement animation for spacebar test
    function showAchievementAnimation(rank, score, presses) {
        ...
        if (rank === 1) {
            icon = '👑';
            title = 'SPACEBAR CHAMPION!';
            message = `You've achieved the #1 spot globally with ${score.toFixed(1)} PPS in ${testDuration} seconds! Your spacebar pressing skills are unmatched.`;
        } else if (rank === 2) {
            icon = '🥈';
            title = 'SILVER MEDALIST!';
            message = `Amazing spacebar skills! You're the 2nd best worldwide with ${score.toFixed(1)} PPS.`;
        } else if (rank === 3) {
            icon = '🥉';
            title = 'BRONZE MEDALIST!';
            message = `Outstanding pressing! You've secured the 3rd position globally with ${score.toFixed(1)} PPS.`;
        } else if (rank <= 4) {
            icon = '⭐';
            title = 'TOP 4 ELITE!';
            message = `Incredible ${testDuration}-second performance! You're among the top 4 spacebar pressers worldwide with ${score.toFixed(1)} PPS.`;
        } else if (rank <= 10) {
            icon = '🏆';
            title = 'TOP 10 MASTER!';
            message = `Excellent spacebar skills! You've made it to the top 10 with ${score.toFixed(1)} PPS. Keep pushing for the top!`;
        }
        ...
        overlay.innerHTML = `
            <div class="achievement-content">
                ...
                <div class="achievement-rank">Rank: #${rank} | Score: ${score.toFixed(1)} PPS | Presses: ${presses} | Duration: ${testDuration}s</div>
